...
use stargaze_anchor::{Provider, ReputationMirrored};
use stargaze_anchor_tests::{
    find_event, ix_ccip_mirror_score, ix_initialize, ix_register_provider, provider_pda, setup_svm,
};

const PROVIDER_ID: [u8; 32] = [11u8; 32];
...
fn authority_updates_score() {
    ...
    let evt: ReputationMirrored = find_event(&meta.logs).expect("ReputationMirrored event");
...
#[test]
fn non_authority_signer_rejected() {
    let (mut svm, authority) = setup_svm();
    ...
    svm.airdrop(&attacker.pubkey(), 1_000_000_000).expect("airdrop");

    let err = send(
        &mut svm,
        &attacker,
        &[&attacker],
        &[ix_ccip_mirror_score(&attacker.pubkey(), PROVIDER_ID, 700)],
    )
...
fn score_out_of_range_rejected() {
    ...
    let err = send(
        ...
        &authority,
        &[&authority],
        &[ix_ccip_mirror_score(&authority.pubkey(), PROVIDER_ID, 1001)],
    )
